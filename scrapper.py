...
import pymongo # importing the pymongo for database
################__________________________________#################################
 ######## this is the portion of mongo db creation and the mongodb connect here #######################
 

myclient = pymongo.MongoClient("mongodb://localhost:27017/")
mydb = myclient["Imageist_DATA"]
mycol = mydb["Imageist_DATA"]
...
class Scrapper: # Scrapper class used to scrape data from the web
    ...
    def get_product_links_diners(self,url):
        ...
        try:    # Try block to handle the exceptions
            ...
            time.sleep(3)   # Waiting for 5 seconds to load the page
    ...
    def get_product_details_diners(self,links,type,gender): 
        ...
        try:
            ...
            for link in links:  # Loop to get the product details from all the links
                ...
                mydict={  # Dictionary to store the data
                    'title':product_title,
                    'price':product_price,
                    'sku':product_sku,
                ...
                filter = {"product_link": mydict["product_link"]}
                # Use update_one() to update the document, or insert a new one if it does not exist
                result = mycol.update_one(filter, {"$set": mydict}, upsert=True)
                print(mydict) # Printing the data
                details.append(mydict)    # Appending the data to the details list
            driver.quit()   # Quitting the driver
    ...
    def get_product_details_equator(self,links,type,gender):
        ...
        try:
            ...
            for link in links:  # Loop to get the product details
                ...
                result = mycol.update_one(filter, {"$set": mydict}, upsert=True)
                print(mydict) # Printing the data
                details.append(mydict)    # Appending the data to the details list            driver.quit()   # Quitting the driver
    ...
    def get_product_details_monark(self,links,type,gender):
        ...
        try:    # Try block to handle the exceptions
            ...
            for link in links:  # Loop to get the product details
                ...
                result = mycol.update_one(filter, {"$set": mydict}, upsert=True)
                print(mydict) # Printing the data
                details.append(mydict)    # Appending the data to the details list            driver.quit()
